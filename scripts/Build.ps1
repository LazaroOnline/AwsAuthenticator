...
dotnet publish "src/AwsCredentialManager/AwsCredentialManager.csproj" -c "Release" -o $publishFolderApp /p:DebugType=None -p:PublishSingleFile=true --self-contained false # -r "win-x64"
Write-Host -F Green "Publishing app DONE!"

$version = (Get-Item "$publishFolderApp/AwsCredentialManager.exe").VersionInfo.FileVersion


$destinationZip = "$publishFolder/AwsCredentialManager-v$version.zip"
